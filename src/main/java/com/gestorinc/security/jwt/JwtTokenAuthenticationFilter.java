...
import static com.gestorinc.exception.enums.Error.HA_OCURRIDO_UN_ERROR_EN_EL_PROCESO_FAVOR_INTENTAR_MÁS_TARDE;
...
public class JwtTokenAuthenticationFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            handleException(response, Error.ERROR_DE_AUTENTICACIÓN_DE_BANCO_TOKEN_NO_VALIDO_O_EXPIRADO);
        }
        catch (RuntimeException e) {
            handleException(response, HA_OCURRIDO_UN_ERROR_EN_EL_PROCESO_FAVOR_INTENTAR_MÁS_TARDE);
        }
    }
    ...
    private void handleException(ServletResponse response, Error error) throws IOException {
        response.getWriter().write(
                new ObjectMapper()
                        .writeValueAsString(
                                new ErrorResponse(ER, error.getCodigo())));
    }
